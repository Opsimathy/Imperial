...
This tutorial is designed for independent study, to illustrate practical aspects of our work. You will find accompanying data files on Blackboard. You are recommended to read Prop 2.29 of the lecture notes before attempting the tutorial. For the second section (Extreme value theory), the tutorial introducing QQ plots will be useful.
...
As we expected, the comparison is not significant. Noting the p-value is around $0.4$, one might ask whether this means, in some sense, that the difference is *very insignificant*. To see what's wrong with this, let's think about what we would expect to see if we carry out a t-test when the null hypothesis is in fact true.
...
1. simulate many normally distributed data sets: one group of size $n$ with mean $0$ and the other group of size $n$ with mean $\delta \in \{ 1,0.1, 0.01,0.001 \}$. Choose $n$ yourself.
